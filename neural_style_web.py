...
CONTENT_IMAGE_FOLDER = '/app/neural-style-tf/content_image'
STYLE_IMAGE_FOLDER = '/app/neural-style-tf/style_image'
RESULT_IMAGE_FOLDER = '/app/neural-style-tf/result_image'
# ALLOWED_EXTENSIONS = set(['png', 'jpg', 'jpeg', 'gif'])
# Only PNG files have been tested.
ALLOWED_EXTENSIONS = set(['png'])
RESULT_FILE = 'result.png'

app = Flask(__name__)
app.debug = True
app.config['CONTENT_IMAGE_FOLDER'] = CONTENT_IMAGE_FOLDER
app.config['STYLE_IMAGE_FOLDER'] = STYLE_IMAGE_FOLDER
app.config['RESULT_IMAGE_FOLDER'] = RESULT_IMAGE_FOLDER
...
def parse_args():
    ...
    parser.add_argument('style_imgs', action='append', type=str,
                        help='Filenames of the style images (example: starry-night.jpg)')

    parser.add_argument('style_imgs_weights', action='append', type=float,
                        default=[1.0],
    ...
    parser.add_argument('content_layers', action='append', type=str,
                        default=['conv4_2'],
                        help='VGG19 layers used for the content image. (default: %(default)s)')

    parser.add_argument('style_layers', action='append', type=str,
                        default=['relu1_1', 'relu2_1', 'relu3_1', 'relu4_1', 'relu5_1'],
                        help='VGG19 layers used for the style image. (default: %(default)s)')

    parser.add_argument('content_layer_weights', action='append', type=float,
                        default=[1.0],
                        help='Contributions (weights) of each content layer to loss. (default: %(default)s)')

    parser.add_argument('style_layer_weights', action='append', type=float,
    ...
    parser.add_argument('style_mask_imgs', action='append', type=str,
                        default=None,
    ...
    parser.add_argument('prev_frame_indices', action='append', type=int,
                        default=[1],
    ...
    args.device = '/cpu:0'
    args.content_img_dir = '/app/neural-style-tf/content_image'
    args.style_imgs_dir = '/app/neural-style-tf/style_image'
    args.img_output_dir = '/app/neural-style-tf/result_image'
    args.verbose = True
    args.content_img = 'content.png'
    args.style_imgs = ['style.png']
    args.video = False
...
def sum_content_losses(sess, net, content_img):
    ...
    for layer, weight in zip(args.content_layers, args.content_layer_weights):
        print('Printing layer next:\n\n')
        print(layer)
        print('Printed layer:\n\n')
        print('Printing net next:\n\n')
        print(net)
        print('Printed net:\n\n')
        print('Printing net[layer] next:\n\n')
        print(net[layer])
        print('Printed net[layer]:\n\n')
...
def main():
    ...
    if file and allowed_file(file.filename):
        file.save(os.path.join(
            app.config['CONTENT_IMAGE_FOLDER'], 'content.png'))
    file = request.files['style_image']
    if file and allowed_file(file.filename):
        file.save(os.path.join(app.config['STYLE_IMAGE_FOLDER'], 'style.png'))
    render_single_image()
    with open('/app/neural-style-tf/result_image/result/result.png', 'rb') as bites:
        return send_file(
            io.BytesIO(bites.read()),
            attachment_filename=app.config['RESULT_FILE'],
            mimetype='image/*'
        )
...
def retrieve(filename):
    return send_from_directory(app.config['RESULT_IMAGE_FOLDER'],
                               filename)
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        file.save(os.path.join(
            app.config['RESULT_IMAGE_FOLDER'], file.filename))
    return "File uploaded"


@app.route('/upload_and_retrieve', methods=['GET', 'POST'])
def upload_and_retrieve_file():
    file = request.files['file']
    if file and allowed_file(file.filename):
        file.save(os.path.join(
            app.config['RESULT_IMAGE_FOLDER'], file.filename))
    with open(os.path.join(app.config['RESULT_IMAGE_FOLDER'], app.config['RESULT_FILE']), 'rb') as bites:
        return send_file(
            io.BytesIO(bites.read()),
            attachment_filename=app.config['RESULT_FILE'],
            mimetype='image/*'
